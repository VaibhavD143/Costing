			...
			<div class='content-page'>
				<div class='content'>
					<div class='container'>
						<div class='row'>
							<div class='col-sm-12'>
								<h4 class='pull-left page-title'>Units List</h4>
						...
						<div class='row'>
							<div class='col-md-12'>
								<div class='grid simple'>
									...
									<?php } ?>
										<a href='<?php echo base_url(); ?>units/insert<?php echo '?nav_menu=units&token='.$_SESSION['token']; ?>' class='btn btn-icon waves-effect waves-light btn-primary m-b-5' data-original-title='Add New' data-toggle='tooltip'> <i class='fa fa-plus'></i> </a>
...
<script type='text/javascript'>
	$(document).ready(function() {
		table = $('#table').DataTable({
			...
			'serverSide': true,
			'ajax': {
				'url': '<?php echo site_url('units/get_all?token=').$_SESSION['token']; ?>',
				'type': 'POST'
			},
			'columnDefs': [{
				'targets': 0,
			...
			},
			{
				...
				'orderable': false,
				'render': function ( data, type, row ) {
					return '<a unit-id="'+row[0]+'" class="cursor view_detail" data-toggle="modal" data-target="#quickview" onclick="return view_detail('+row[0]+')"><i class="fa fa-eye"></i></a>';
				},
			},
			{
				'targets': 3,
				...
				'orderable': false,
				'render': function ( data, type, row ) {
					return '<a unit-id="'+row[0]+'" href="javascript:void(0);" class="edit"><i class="fa fa-pencil"></i></a>';
				},
			},
			{
				'targets': 4,
				...
				'orderable': false,
				'render': function ( data, type, row ) {
					return '<a unit-id="'+row[0]+'" href="javascript:void(0);" class="single_delete"><i class="fa fa-trash"></i></a>';
				},
			}],
		});
	});
	/*------------------------------------------On Click Edit--------------------------------------*/
		$(document).on('click', '.edit', function(){ 
			...
			window.location.href='<?php echo site_url('units/update?token=').$_SESSION['token']; ?>'+'&unit_id='+unit_id; 
		});
	/*------------------------------------------On Click Single Delete Box--------------------------------------*/
		$(document).on('click', '.single_delete', function(){
			...
			if (confirm('Are you sure, you want to delete?')) {
				...
				$.ajax({
					type: 'GET',
					url: '<?php echo site_url('units/delete?token=').$_SESSION['token']; ?>'+'&unit_id='+unit_id,
					dataType: 'json',
		...
		$(document).on('click', '.delete_all', function(){ 
			...
			if (confirm('Are you sure, you want to delete?')) {
				$('.dataTables_processing').show();
					$.ajax({
						type: 'GET',
						url: '<?php echo site_url('units/delete_all?token=').$_SESSION['token']; ?>'+'&unit_id='+unit_id,
						dataType: 'json',
						success: function(data){
							console.log(JSON.stringify(data));
							if(data.success) {
	...
	/*------------------------------------------On Click View Detail--------------------------------------*/
		function view_detail(unit_id) {
				$.ajax({
					type: 'GET',
					url: '<?php echo site_url('units/get_by_id?token=').$_SESSION['token']; ?>'+'&unit_id='+unit_id,
					dataType: 'json',
					success: function(data){
						$('.details').html('');
						var optionhtml = '';
						...
						optionhtml = '<div class="text-center">'
							optionhtml += '<table class="table table-striped table-bordered">';
								optionhtml += '<thead>';
									optionhtml += '<tr>';
										optionhtml += '<th>Unit Name</th>';
										optionhtml += '<td>'+data.data[0].unit_name+'</td>';
								optionhtml += '</thead>';
							optionhtml += '</table>';
						optionhtml += '</div>';
